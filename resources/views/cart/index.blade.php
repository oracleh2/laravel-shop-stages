    ...
    <main class="py-16 lg:py-20">
        <div class="container">
            ...
            <ul class="breadcrumbs flex flex-wrap gap-y-1 gap-x-4 mb-6">
                <li><a href="{{ route('index') }}l" class="text-body hover:text-pink text-xs">Главная</a></li>
            ...
            <section>
                ...
                @else
                        ...
                        <table class="min-w-full border-spacing-y-4 text-white text-sm text-left" style="border-collapse: separate">
                            ...
                            @foreach($items as $item)
                                ...
                                <td scope="row" class="py-4 px-4 md:px-6 rounded-l-2xl bg-card">
                                    <div class="flex flex-col lg:flex-row min-w-[200px] gap-2 lg:gap-6">
                                        <div class="shrink-0 overflow-hidden w-[64px] lg:w-[84px] h-[64px] lg:h-[84px] rounded-2xl">
                                            <img src="{{ $item->product->thumbnail }}" class="object-cover w-full h-full" alt="{{ $item->product->title }}">
                                        </div>
                                        <div class="py-3">
                                ...
                                <td class="py-4 px-4 md:px-6 bg-card">
                                    <div class="font-medium whitespace-nowrap">{{ $item->price->cartPrice() }}</div>
                                </td>
                                <td class="py-4 px-4 md:px-6 bg-card">
                                    <div class="flex items-stretch h-[56px] gap-2">
                                        <form action="{{ route('cart.quantity', $item) }}" method="post">
                                            ...
                                            <input name="quantity" value="{{ $item->quantity }}" type="number" class="h-full px-2 lg:px-4 rounded-lg border border-body/10 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition" min="1" max="999"  placeholder="К-во">
                                            <button type="button" class="w-12 h-full rounded-lg border border-body/10 hover:bg-card/20 active:bg-card/50 focus:border-pink focus:shadow-[0_0_0_3px_#EC4176] bg-white/5 text-white text-xs text-center font-bold shadow-transparent outline-0 transition">+</button>
                                        </form>
                                    </div>
                                </td>
                                <td class="py-4 px-4 md:px-6 bg-card">
                                    <div class="font-medium whitespace-nowrap">{{ $item->amount->cartPrice() }}</div>
                                </td>
                    ...
                    <div class="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-4 mt-8">
                        <div class="text-[32px] font-black">Итого: {{ cart()->totalAmount()->cartPrice() }}</div>
                        ...
                        <div class="flex flex-col sm:flex-row lg:justify-end gap-4">
                            ...
                            <a href="{{ route('order.index') }}" class="btn btn-purple">Оформить заказ</a>
                        </div>
                    </div>
                @endif
            </section>
