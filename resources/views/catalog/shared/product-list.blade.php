<div class="product-card flex flex-col md:flex-row rounded-3xl bg-card">
    <a href="{{ route('product', [ 'product' => $product ]) }}" class="product-card-photo overflow-hidden shrink-0 md:w-[260px] xl:w-[320px] h-[320px] md:h-full rounded-3xl">
    ...
    <div class="grow flex flex-col py-8 px-6 md:px-8">
        <h3 class="text-sm lg:text-md font-black">
            <a href="{{ route('product', [ 'product' => $product ]) }}" class="inline-block text-white hover:text-pink">
                {{ $product->title }}
            </a>
        </h3>
        ...
        <div class="flex flex-col xl:flex-row xl:items-center xl:justify-between gap-6 mt-6">
            <div class="flex items-baseline gap-4">
                <div class="text-pink text-md xl:text-lg font-black">{{ $product->price->price() }}</div>
