            ...
            <section class="mt-16 lg:mt-24">
                ...
                <div class="flex flex-col lg:flex-row gap-12 lg:gap-6 2xl:gap-8 mt-8">
                    ...
                    <aside class="basis-2/5 xl:basis-1/4">
                        <form action="{{ route('catalog') }}" class="overflow-auto max-h-[320px] lg:max-h-[100%] space-y-10 p-6 2xl:p-8 rounded-2xl bg-card">
                    ...
                    <div class="basis-auto xl:basis-3/4">
                            ...
                            <div class="flex items-center gap-4">
                                <div class="flex items-center gap-2">
                                    <a
                                        ...
                                        @class([
                                            'inline-flex items-center justify-center w-10 h-10 rounded-md bg-card hover:text-pink',
                                            'pointer-events-none text-pink' => is_catalog_view('grid')
                                        ])
                                    >
                                    ...
                                    <a
                                        ...
                                        @class([
                                            'inline-flex items-center justify-center w-10 h-10 rounded-md bg-card hover:text-pink',
                                            'pointer-events-none text-pink' => is_catalog_view('list')
                                        ])
                                    >
                                        <svg class="w-5 h-5" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 52 52">
                                ...
                                <div class="text-body text-xxs sm:text-xs">Найдено: {{ $products->total() }} {{ trans_choice('товар|товара|товаров', $products->total(), [], 'ru') }}</div>
                            </div>
                            <div x-data="{sort: '{{ filter_url($category, ['sort' => request('sort'), 'q' => request('q')]) }}', q: '{{ request('q') }}'}" class="flex flex-col sm:flex-row sm:items-center gap-3">
                                ...
                                <form x-ref="sortForm" action="{{ route('catalog', $category) }}">
                                    ...
                                    <select
                                        ...
                                        <option value="{{ filter_url($category, ['sort' => null]) }}" class="text-dark">умолчанию</option>
                                        <option @selected( request('sort') == 'price') value="{{ filter_url($category, ['sort' => 'price']) }}" class="text-dark">от дешевых к дорогим</option>
                                        <option @selected( request('sort') == '-price') value="{{ filter_url($category, ['sort' => '-price']) }}" class="text-dark">от дорогих к дешевым</option>
                                        <option @selected( request('sort') == 'title') value="{{ filter_url($category, ['sort' => 'title']) }}" class="text-dark">наименованию (A - Z)</option>
                                        <option @selected( request('sort') == '-title') value="{{ filter_url($category, ['sort' => '-title']) }}" class="text-dark">наименованию (Z - A)</option>
                                    </select>
                                </form>
                            </div>
                        </div>
                        <div
                            @class([
                                'products grid grid-cols-1 gap-y-8',
                                'sm:grid-cols-2 xl:grid-cols-3 gap-x-6 2xl:gap-x-8 lg:gap-y-10 2xl:gap-y-12' => is_catalog_view('grid'),
                            ])
                            >
                            @foreach($products as $product)
                                @include('catalog.shared.product' . (is_catalog_view('list') ? '-list' : ''), ['product' => $product, 'category' => $category])
                            @endforeach
                        </div>
                        <div class="mt-12">
